...
router.get("/searc-hmedical-history/:userId", async (req, res) => {
    try {
        ...
        const medicalHistoryRecord = await MedicalHistory.findOne({
            user: userId,
        });
...
router.patch("/update-medical-history/:userId", async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: 1,
            data: updatedRecord,
            message: "Medical history record update successfully",
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Internal Server Error" });
...
router.delete("/delete-medical-history/:userId", async (req, res) => {
    try {
        ...
        const deletedRecord = await MedicalHistory.findOneAndDelete({
            user: userId,
        });
...
router.get("/search-count/:userId", async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: 1,
            searchCount: medicalHistoryRecord.searchCount,
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Internal Server Error" });
